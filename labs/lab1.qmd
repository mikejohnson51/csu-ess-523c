...
2. Instatiate it with a git (`usethis::use_git()`) archive and Github repo (`usethis::use_github()`)
4. Create a new .qmd file called `lab-01.qmd`
...
With public data— particularly in environmental and health sciences — no longer carrying the same [promised persistence it once did](https://www.muckrock.com/news/archives/2025/feb/10/federal-data-is-disappearing-on-thursday-meet-the-teams-working-t o-rescue-it-and-learn-how-you-can-help/), the importance of external archiving efforts by journalists, scientists, and researchers has grown. The availability of reliable, well-documented datasets is crucial for advancing environmental science, informing policy, and fostering transparency.
...
a. Start by reading in the data from the NY-Times URL with `read_csv` (make sure to attach the `tidyverse`)
...
# **Question 3**: Evaluating Census Data (EDA)
...
# **Question 8**: Space & Time

You've now been tasked with understanding how COVID has pread through time across the country. You will do this by calculating the Weighted Mean Center of the COVID-19 outbreak to better understand the movement of the virus through time. 
...
- For each date, calculate the Weighted Mean $X_{coord}$ and $Y_{coord}$ using the daily cumulative cases and the weight $w_{i}$. 

```{r}

```

- Plot the weighted mean center (`aes(x = LNG, y = LAT)`) sized by total cases for each day. These points should be plotted over a map of the USA states which can be added to a ggplot object with:
...
# **Question 9:** Cases vs. Deaths

Extend your analysis to also compute the weighted mean center of daily COVID deaths. 
...
OK! This is a job well done. As your final task, your leadership has noticed that it is much easier to have a solid record of deaths, while a record of cases relies on testing protocols and availability. They ask you to explore the relationship between cases and deaths to see if deaths can be used as a proxy for cases. You will explore the relationship between daily new cases and daily new deaths along with other predictors of your chosing from the population data.

Build a linear model that best tries to estimate the death toll. Is nesting useful? What about interaction terms?
...
Once your model is built, evaluate it using the `broom` package. Report the R-squared value and the p-value of the model and what this means for the the value of its application.
