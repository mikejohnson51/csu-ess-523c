...
- It includes functions for pivotingand nesting data.
...
- It is useful when you want to import spatial data into R.
...
- The `mask()` function is used to mask a raster using a vector or other extent

```{r}
larimer_terra <- project(vect(larimer), "EPSG:5070")
larimer_elev = crop(elev, larimer_terra)
plot(larimer_elev)

larimer_mask = mask(larimer_elev, larimer_terra)
plot(larimer_mask)
```
...
```{r}
clean_penguins = drop_na(penguins)
model <- lm(body_mass_g ~ flipper_length_mm, data = clean_penguins)
summary(model)
```
...
- The `parsnip` package is used for model selection.
- It provides functions for defining models and workflows.
...
## Workflowsets for model comparison

```{r}
(workflowset <- workflow_set(list(penguin_recipe), list(log_reg_model, rf_model, dt_model)) |> 
...
validation <- fit(final_model, data = penguins_test)

augment(validation, penguins_test) |> 
...
- Today we reviewd/introduced the basics of R for environmental data science.
- We discussed data types, structures, and packages for data manipulation.
- We also explored modeling, spatial data, and raster data.
